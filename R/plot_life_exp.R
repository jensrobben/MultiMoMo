...
#'
#' @description This function plots the estimated period and/or cohort life
...
#' @param age The age.
#' @param le_sim The estimated period and/or cohort life expectancies.
...
#'
#' @return A ggplot representing the evolution and the uncertaincy of the period 
...
plot_life_exp <- function(le_yv, age, le_sim, sex, type, quantile, m_obs = NULL){
  ...
  if("per" %in% type & ! is.null(m_obs)){
    ...
    df_per_le <- data.frame("Values" = unname(df_per_le[,1]),
                            "Years" = as.numeric(rownames(df_per_le)))
    p <- p + geom_point(mapping = aes(x = Years, y = Values, col = "dots"),
                   data = df_per_le) + 
